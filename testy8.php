...
<?php foreach ($topWorks as $i => $workPath):
    $work = $workDetails[$workPath];
    ?>
    <div class="selected-work-card" data-idx="<?php echo $i; ?>" style="cursor:pointer; min-width:130px; max-width:160px; flex:0 0 auto; background:#f9f9f9; border-radius:14px; box-shadow:0 4px 14px #0001; padding: 12px;">
        <img src="<?php echo htmlspecialchars($work['path']); ?>" alt="<?php echo htmlspecialchars($work['title']); ?>" style="width:100%; max-width:140px; max-height:110px; object-fit:cover; border-radius:8px;">
        <div style="margin-top:8px;font-size:0.9em;font-weight:bold;"><?php echo htmlspecialchars($work['title']); ?></div>
    </div>
<?php endforeach; ?>
</div>
...
function openModalForSlideshow(slideIdx) {
    ...
    if (profile) {
        ...
        if (visitProfileBtn) {
            visitProfileBtn.onclick = function() { window.location.href = "profile.php?user="+encodeURIComponent(workData.user_folder);};
            visitProfileBtn.style.display = 'block';
        }
    } else {
        if (modalArtist) modalArtist.textContent = "Unknown Artist";
        if (visitProfileBtn) {
            visitProfileBtn.onclick = null;
